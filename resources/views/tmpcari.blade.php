...
<div id="sidebar">
    ...
    <div class="global-search-container">
        <div class="global-search-box">
            ...
            <input type="text" id="global-search-input" placeholder="Cari layer atau nama lokasi... (contoh: jatigede)">
...
<script>
    ...
    function toggleWFS(checkbox, layerName, category, subCategory = null) {
        if (checkbox.checked) {
            ...
            fetch(url).then(res => res.json()).then(data => {
                ...
                // Simpan ke cache untuk pencarian
    ...
    function loadSidebarLayers() {
        ...
        const mainCategories = ["DAS dan Jaringan Sungai", "Infrastruktur", "Daerah Kewenangan Kabupaten dan Provinsi", "Daerah Irigasi Pusat", "Kebencanaan", "Aset Tanah", "Sumber Daya Alam"];
    ...
    function performGlobalSearch(query) {
        ...
        // Cari lokasi dari cache
        allLocationsCache.forEach(loc => {
            if (loc.name.toLowerCase().includes(query)) {
                if (!results.some(r => r.type === 'location' && r.name === loc.name)) {
                    results.push({ type: 'location', title: loc.layerTitle, name: loc.name, coords: loc.coords });
                }
            }
        });
            ...
            results.forEach(res => {
                ...
                item.onclick = () => {
                    if (res.type === 'layer') {
                        ...
                        searchMarker.bindPopup(`<div style="text-align:center;"><strong>${res.name}</strong><br>${res.title}</div>`).openPopup();
                        ...
                        searchMarker.bindPopup(`<div style="text-align:center;"><strong>Koordinat</strong><br>Lat: ${res.lat}<br>Lng: ${res.lng}</div>`).openPopup();
