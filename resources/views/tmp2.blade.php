...
<head>
    ...
    <style>
        * {
        ...
        /* Hanya animasi saat hover untuk mengurangi beban CPU */
        .lokasi-kritis-simona-marker div {
            transition: transform 0.2s ease;
        }
        
        .lokasi-kritis-simona-marker:hover div {
            transform: scale(1.5);
        }
...
<div id="map"></div>
<div id="layer-loading" class="layer-loading">Memuat layer...</div>

<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<script>
    // Optimasi untuk HP: deteksi device
    ...
    function showLoading(show) {
        const loader = document.getElementById('layer-loading');
        if (loader) {
    ...
    function createSearchIcon() {
        return L.divIcon({
            html: `<div style="position: relative;">
                        ...
                        <div style="position: absolute; top: -8px; left: 8px; width: 8px; height: 8px; background: #e74c3c; border-radius: 50%;"></div>
                    </div>`,
    ...
    function createMarkerIcon(color, layerName) {
        ...
        const svgIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="28" height="28"><path fill="${color}" stroke="#fff" stroke-width="1" d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>`;
        return L.divIcon({ html: svgIcon, className: 'custom-pin', iconSize: [28, 28], iconAnchor: [14, 28], popupAnchor: [0, -28] });
    }
    ...
    function toggleWFS(checkbox, layerName, category, subCategory = null) {
        if (checkbox.checked) {
            if (layerCache.has(layerName)) {
                const cachedLayer = layerCache.get(layerName);
                cachedLayer.addTo(map);
                activeLayers[layerName] = cachedLayer;
                if (layerInfoForLegend[layerName]) {
                    updateLegend();
                }
            ...
            fetch(url, { signal: controller.signal })
                ...
                .then(data => {
                    ...
                    if (!data.features || data.features.length === 0) {
                        showToast(`Tidak ada data untuk ${formatTitle(layerName)}`, 'warning');
                    ...
                    if (iconPath) { 
                        ...
                        const featuresToProcess = data.features.slice(0, isMobile ? 1000 : 3000);
                        ...
                        if (Object.keys(classes).length > 1) { 
                            ...
                            info.classes = Object.values(classes); 
                        } else {
                            info.type = 'single'; 
                            info.color = "#65f176";
                        }
                    ...
                    var geoJsonLayer = L.geoJson(data, {
                        ...
                        onEachFeature: (feature, layer) => {
                            ...
                            for (let key in props) { 
                                ...
                                rows += `<tr><td style="width:100px"><b>${formatTitle(key)}</b>NonNull<td>${props[key] || "-"}</td></tr>`;
                            }
                            ...
                            if (primaryName && allValues.length > 0) {
                                ...
                                if (feature.geometry.type === 'Point') {
                                    ...
                                    try { coords = [feature.geometry.coordinates[0][0][1], feature.geometry.coordinates[0][0][0]]; } catch(e){}
                                } else if (feature.geometry.type === 'MultiPolygon') {
                                    try { coords = [feature.geometry.coordinates[0][0][0][1], feature.geometry.coordinates[0][0][0][0]]; } catch(e){}
                                }
                                if (coords && !allLocationsCache.some(l => l.name === String(primaryName))) {
                    ...
                    if (geoJsonLayer.getBounds && geoJsonLayer.getBounds().isValid()) {
                        map.flyToBounds(geoJsonLayer.getBounds(), { padding: [50, 50], duration: 1 });
                    }
                ...
                .catch(err => {
                    clearTimeout(timeoutId);
                    console.error("Gagal memuat layer WFS:", err);
                    showToast(`Gagal memuat ${formatTitle(layerName)}`, 'error');
                    checkbox.checked = false;
                    showLoading(false);
                });
    ...
    // ==================== LOKASI KRITIS SIMONA (TANPA ANIMASI PULSE) ====================
    ...
    // VERSION TANPA ANIMASI - HANYA MARKER STATIS
    function getSimonaIcon(tingkatKerusakan) {
        var color = getColorByTingkatKerusakan(tingkatKerusakan);
        
        // Marker statis tanpa animasi pulse
        return L.divIcon({
            html: `<div style="width: 14px; height: 14px; background: ${color}; border: 2px solid white; border-radius: 50%; box-shadow: 0 0 4px rgba(0,0,0,0.3);"></div>`,
                ...
                if (features.length > 0) {
                    ...
                    lokasiKritisSimona = L.geoJson({
                        ...
                        pointToLayer: function(feature, latlng) {
                            ...
                            lokasiKritisSimonaMarkers.push(marker);
                            return marker;
                        },
                        onEachFeature: function(feature, layer) {
                            ...
                            for (let field of fields) {
                                if (props[field.key] && props[field.key] !== '' && props[field.key] !== '-') {
                                    rows += `<tr><td style="width:100px"><b>${field.label}</b>NonNull<td>${props[field.key]}</td></tr>`;
                                }
                            }
    ...
    function loadSidebarLayers() {
        ...
        fetch(capabilitiesUrl)
            ...
            .then(data => {
                ...
                document.getElementById('dynamic-menu').innerHTML = '<p style="color:red; text-align:center;">Gagal memuat data. Periksa koneksi internet.</p>';
            });
    }
...
// Tambahkan fungsi ini untuk mendapatkan warna yang konsisten antara peta dan legenda
function getLayerColor(layerName, feature = null) {
    let lowerLayer = layerName.toLowerCase();
    
    // Logika khusus untuk jaringan sungai
    if (lowerLayer.includes('jaringan') && feature) {
        let props = feature.properties || {};
        let selectedVal = props.saluran || props.Saluran || props.SALURAN || 
                          props.n_aset || props.N_Aset || props.N_ASET ||
                          props.nama || props.Nama || props.NAMA;
        if (selectedVal) {
            let val = String(selectedVal).toLowerCase().trim();
            if (val.includes('induk')) return classificationColors['saluran induk'] || '#000000';
            if (val.includes('primer')) return classificationColors['saluran primer'] || '#ff0000';
            if (val.includes('sekunder')) return classificationColors['saluran sekunder'] || '#ffea00';
            if (val.includes('tersier') || val.includes('terrsier')) return classificationColors['saluran tersier'] || '#0015f8';
            if (val.includes('pembuang')) return classificationColors['saluran pembuang'] || '#93f2f2';
            if (val.includes('suplesi')) return classificationColors['saluran suplesi'] || '#ea6500';
        }
    }
    
    // Cek langsung di classificationColors
    if (classificationColors[lowerLayer]) {
        return classificationColors[lowerLayer];
    }
    
    // Cek berdasarkan kata kunci
    if (lowerLayer.includes('sempadan')) return '#12d0f1';
    if (lowerLayer.includes('lokasi_kritis')) return '#800000';
    if (lowerLayer.includes('baku')) return classificationColors['baku'] || '#006400';
    if (lowerLayer.includes('fungsional')) return classificationColors['fungsional'] || '#7CFC00';
    if (lowerLayer.includes('potensial')) return classificationColors['potensial'] || '#32CD32';
    if (lowerLayer.includes('waduk_ar')) return '#0070FF';
    if (lowerLayer === 'das_cimanggis_ar') return "#FF7F7F";
    if (lowerLayer.includes('sungai')) return "#3498db";
    
    return "#65f176"; // default
}

// Perbaiki fungsi getFeatureStyle
function getFeatureStyle(feature, layerName) {
    let color = getLayerColor(layerName, feature);
    let fOpacity = 0.6;
    let lWeight = 1.5;
    let lowerLayer = layerName.toLowerCase();
    ...
    if (lowerLayer.includes('jaringan')) {
        ...
        if (selectedVal) {
            let val = String(selectedVal).toLowerCase().trim();
            if (val.includes('induk')) lWeight = 4.5;
            else if (val.includes('primer')) lWeight = 3.5;
            else if (val.includes('sekunder')) lWeight = 2.5;
            else if (val.includes('tersier') || val.includes('terrsier')) lWeight = 1.5;
        }
    }
    
    if (lowerLayer.includes('sempadan')) {
        fOpacity = 0.1;
        return { color: color, weight: 3, fillOpacity: fOpacity, opacity: 1, fillColor: color };
    }
    
    if (lowerLayer === 'das_cimanggis_ar') {
        fOpacity = 0;
        lWeight = 3;
    }
...
// Perbaiki fungsi toggleWFS untuk legenda yang konsisten
function toggleWFS(checkbox, layerName, category, subCategory = null) {
    if (checkbox.checked) {
        if (layerCache.has(layerName)) {
            const cachedLayer = layerCache.get(layerName);
            ...
            showToast(`${formatTitle(layerName)} dimuat dari cache`, 'success');
            return;
        }
        ...
        fetch(url, { signal: controller.signal })
            ...
            .then(data => {
                ...
                if (iconPath) { 
                    ...
                    // Kumpulkan warna yang digunakan untuk legenda
                    let colorClasses = new Map();
                    const isJaringanLayer = layerName.toLowerCase().includes('jaringan') || layerName.toLowerCase().includes('saluran');
                    
                    const featuresToProcess = data.features.slice(0, isMobile ? 1000 : 3000);
                    
                    featuresToProcess.forEach(f => {
                        let color = getLayerColor(layerName, f);
                        let label = null;
                        
                        // Dapatkan label untuk legenda
                        if (isJaringanLayer) {
                            ...
                            if (selectedVal) {
                                let val = String(selectedVal).toLowerCase().trim();
                                if (val.includes('induk')) label = 'Saluran Induk';
                                else if (val.includes('primer')) label = 'Saluran Primer';
                                else if (val.includes('sekunder')) label = 'Saluran Sekunder';
                                else if (val.includes('tersier')) label = 'Saluran Tersier';
                                else if (val.includes('pembuang')) label = 'Saluran Pembuang';
                                else if (val.includes('suplesi')) label = 'Saluran Suplesi';
                            }
                        }
                        
                        if (!label) {
                            // Cari label dari properties
                            for (let prop in f.properties) {
                                let rawVal = String(f.properties[prop]);
                                let cleanVal = rawVal.toLowerCase().replace(/,/g, '').replace(/\s+/g, ' ').replace(/\(.*\)/g, '').trim();
                                if (classificationColors[cleanVal]) {
                        ...
                        if (!label) label = formatTitle(layerName);
                        
                        if (!colorClasses.has(color)) {
                            colorClasses.set(color, { color: color, label: label });
                        }
                    });
                    
                    if (colorClasses.size > 1) {
                        info.type = 'classified';
                        info.classes = Array.from(colorClasses.values());
                    } else {
                        info.type = 'single';
                        info.color = colorClasses.size > 0 ? Array.from(colorClasses.keys())[0] : getLayerColor(layerName);
                    }
                }
                ...
                var geoJsonLayer = L.geoJson(data, {
                    style: (feature) => getFeatureStyle(feature, layerName),
                    pointToLayer: (feature, latlng) => L.marker(latlng, { icon: createMarkerIcon(getLayerColor(layerName, feature), layerName) }),
                    onEachFeature: (feature, layer) => {
                        let props = feature.properties;
                        let displayTitle = props.Nama || props.nama || props.N_Aset || props.n_aset;
                        if (!displayTitle || displayTitle === "-" || displayTitle === "0") {
                        ...
                        for (let key in props) { 
                            if (key.toLowerCase().includes('id')) continue;
                            if (rowCount++ > 15) {
                                rows += `<tr><td colspan="2" style="text-align:center">... dan atribut lainnya ...</td></tr>`;
                                break;
                            }
                            rows += `<tr><td style="width:100px"><b>${formatTitle(key)}</b>NonNull<td>${props[key] || "-"}</td></tr>`;
                        }
                        layer.bindPopup(`<div class="popup-header">${String(displayTitle).toUpperCase().substring(0, 100)}</div><div class="popup-scroll"><table class="popup-table">${rows}</table></div>`);
                        ...
                        for (let key in props) {
                            let val = props[key];
                            if (val && typeof val === 'string' && val !== "-" && val !== "0" && val.length > 2 && val.length < 200) {
                                allValues.push(val.substring(0, 100));
                                let keyLower = key.toLowerCase();
                                if (keyLower === 'nama' || keyLower === 'n_aset' || keyLower === 'saluran' || 
                                    keyLower.includes('waduk') || keyLower.includes('bendung') || keyLower.includes('lokasi') ||
                                    keyLower === 'namobj' || keyLower === 'aset') {
                                    if (!primaryName) primaryName = val.substring(0, 100);
                                }
                            }
                        }
                        if (!primaryName && allValues.length > 0) primaryName = allValues[0];
                        ...
                        if (primaryName && allValues.length > 0) {
                            ...
                            if (coords && !allLocationsCache.some(l => l.name === String(primaryName))) {
                                // Batasi cache untuk HP
                                if (allLocationsCache.length < (isMobile ? 500 : 2000)) {
                                    allLocationsCache.push({
                                        name: String(primaryName),
                                        allNames: allValues.slice(0, 5),
                                        layerTitle: formatTitle(layerName),
                                        coords: coords
                                    });
                                }
    ...
    var locationUpdateThrottle = false;
    function onLocationFound(e) {
        if (locationUpdateThrottle) return;
        locationUpdateThrottle = true;
        setTimeout(() => { locationUpdateThrottle = false; }, 2000);
