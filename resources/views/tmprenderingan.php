...
<script>
    ...
    // Gunakan basemap yang lebih ringan untuk HP
    ...
    // Pilih basemap default berdasarkan device
    if (isMobile) {
        osm.addTo(map); // OSM lebih ringan dari Google Satellite
    } else {
        googleSat.addTo(map);
    }
    ...
    var layerCache = new Map(); // Cache untuk layer yang sudah dimuat
    
    var searchMarker = null;
    var allLocationsCache = []; // Hanya untuk search, diisi secara lazy
    var availableLayersList = [];
    
    // Marker cluster group untuk layer point
    var markerClusterGroup = L.markerClusterGroup({
        maxClusterRadius: 50,
        spiderfyOnMaxZoom: false,
        showCoverageOnHover: false,
        zoomToBoundsOnClick: true,
        disableClusteringAtZoom: isMobile ? 12 : 14
    });
    ...
    function toggleWFS(checkbox, layerName, category, subCategory = null) {
        if (checkbox.checked) {
            ...
            // Tambahkan timeout untuk jaringan lemah
            ...
            fetch(url, { signal: controller.signal })
                ...
                .then(data => {
                    ...
                    if (iconPath) { 
                        ...
                        // Batasi iterasi untuk performa
                        const featuresToProcess = data.features.slice(0, isMobile ? 500 : 2000);
                    ...
                    // Gunakan marker cluster untuk point layers
                    const isPointLayer = data.features[0]?.geometry?.type === 'Point';
                    let geoJsonLayer;
                    
                    if (isPointLayer && !layerName.toLowerCase().includes('jaringan')) {
                        // Untuk point layer, gunakan marker cluster
                        ...
                        geoJsonLayer = L.geoJson(data, {
                            ...
                            onEachFeature: (feature, layer) => {
                                ...
                                for (let key in props) { 
                                    if (key.toLowerCase().includes('id')) continue;
                                    rows += `<td><td style="width:100px"><b>${formatTitle(key)}</b></td><td>${props[key] || "-"}</td></tr>`; 
                                }
                                layer.bindPopup(`<div class="popup-header">${displayTitle.toUpperCase()}</div><div class="popup-scroll"><table class="popup-table">${rows}</table></div>`);
                            }
                        });
    ...
    function getColorByTingkatKerusakan(tingkat) {
        if (!tingkat) return '#3498db';
        tingkat = tingkat.toLowerCase();
        if (tingkat.includes('berat')) return '#e74c3c';
        if (tingkat.includes('sedang')) return '#f1c40f';
        if (tingkat.includes('ringan')) return '#3498db';
        return '#3498db';
    }
        ...
        fetch('/api.php/api/lokasi-kritis-mysql')
            ...
            .then(data => {
                ...
                if (features.length > 0) {
                    ...
                    // Gunakan marker cluster untuk SIMONA
                    const simonaCluster = L.markerClusterGroup({
                        maxClusterRadius: 40,
                    ...
                    lokasiKritisSimona.eachLayer(layer => simonaCluster.addLayer(layer));
                    simonaCluster.addTo(map);
                    lokasiKritisSimona = simonaCluster;
    ...
    function loadSidebarLayers() {
        ...
        fetch(capabilitiesUrl)
            ...
            .then(data => {
                ...
                // HAPUS preloadAllData() - TIDAK ADA PRELOAD LAGI!
    ...
    // Search dengan debounce untuk performa
    ...
    function performGlobalSearch(query) {
        ...
        // Search hanya dari cache yang sudah ada (tidak preload semua)
        ...
        if (results.length === 0) { 
            ...
            results.slice(0, 20).forEach(res => { // Batasi hasil search
    ...
    // Location tracking dengan throttle
