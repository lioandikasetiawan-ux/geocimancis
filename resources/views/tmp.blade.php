...
<script>
    ...
    function getFeatureStyle(feature, layerName) {
        ...
        color = '#12d0f1';
        lWeight = 3;
        fOpacity = 0.1;  // transparan agar terlihat seperti garis tepi
                ...
                var geoJsonLayer = L.geoJson(data, {
                    ...
                    onEachFeature: (feature, layer) => {
                        ...
                        for (let key in props) { 
                            ...
                            rows += `<table><td style="width:100px"><b>${formatTitle(key)}</b></td><td>${props[key] || "-"}</td></tr>`; 
                        }
    ...
    // PRELOAD SEMUA DATA UNTUK PENCARIAN
...
var lokasiKritisLayer = null;
var lokasiKritisMarkers = [];

function loadLokasiKritis() {
    // Tampilkan loading
    showToast('Memuat data lokasi kritis...', 'info');
    
    fetch('/api/lokasi-kritis')
        .then(response => response.json())
        .then(data => {
            if (data.features && data.features.length > 0) {
                // Hapus layer lama jika ada
                if (lokasiKritisLayer) {
                    map.removeLayer(lokasiKritisLayer);
                }
                
                // Buat icon custom untuk lokasi kritis
                var lokasiKritisIcon = L.divIcon({
                    html: `<div style="position: relative;">
                                <i class="fas fa-exclamation-triangle" style="color: #e74c3c; font-size: 24px; text-shadow: 0 1px 2px rgba(0,0,0,0.3);"></i>
                                <div style="position: absolute; top: -4px; left: 12px; width: 8px; height: 8px; background: #e74c3c; border-radius: 50%;"></div>
                            </div>`,
                    className: 'lokasi-kritis-marker',
                    iconSize: [24, 24],
                    iconAnchor: [12, 24],
                    popupAnchor: [0, -24]
                });
                ...
                // Buat GeoJSON layer
                lokasiKritisLayer = L.geoJson(data, {
                    pointToLayer: function(feature, latlng) {
                        return L.marker(latlng, { icon: lokasiKritisIcon });
                    },
                    onEachFeature: function(feature, layer) {
                        let props = feature.properties;
                        let popupContent = `
                            <div class="popup-header">
                                <i class="fas fa-exclamation-triangle"></i> LOKASI KRITIS
                            </div>
                            <div class="popup-scroll">
                                <table class="popup-table">
                                    <tr><td style="width:100px"><b>Kode</b></td><td>${props.kode || '-'}</td></tr>
                                    <tr><td><b>Desa</b></td><td>${props.desa || '-'}</td></tr>
                                    <tr><td><b>Kecamatan</b></td><td>${props.kecamatan || '-'}</td></tr>
                                    <tr><td><b>Kab/Kota</b></td><td>${props.kab_kota || '-'}</td></tr>
                                    <tr><td><b>Tingkat Kerusakan</b></td><td>${props.tingkat_kerusakan || '-'}</td></tr>
                        ...
                        // Simpan marker untuk fitur zoom
                        lokasiKritisMarkers.push(layer);
                    }
                });
                
                // Tambahkan ke map
                lokasiKritisLayer.addTo(map);
                
                // Simpan ke activeLayers
                activeLayers['lokasi_kritis'] = lokasiKritisLayer;
                
                // Update legend
                layerInfoForLegend['lokasi_kritis'] = {
                ...
                showToast(`${data.features.length} titik lokasi kritis berhasil dimuat`, 'success');
            } else {
                showToast('Tidak ada data lokasi kritis', 'info');
            }
        })
        .catch(error => {
            console.error('Gagal memuat lokasi kritis:', error);
            showToast('Gagal memuat data lokasi kritis', 'error');
        });
}

function toggleLokasiKritis() {
    if (lokasiKritisLayer && map.hasLayer(lokasiKritisLayer)) {
        map.removeLayer(lokasiKritisLayer);
        document.getElementById('lokasi-kritis-checkbox').checked = false;
    } else {
        loadLokasiKritis();
        document.getElementById('lokasi-kritis-checkbox').checked = true;
    }
}

function zoomToLokasiKritis() {
    if (lokasiKritisMarkers.length > 0) {
        var group = L.featureGroup(lokasiKritisMarkers);
        map.fitBounds(group.getBounds(), { padding: [50, 50] });
    } else {
        showToast('Tidak ada marker lokasi kritis', 'info');
    }
}
    // PENCARIAN
                    ...
                    if (res.type === 'layer') {
                        ...
                        searchMarker.bindPopup(`
                            <div style="text-align:center;">
                                <strong>${res.name}</strong><br>${res.title}
                                <br><button onclick="clearSearchMarker()" style="margin-top:8px; padding:4px 12px; background:#e74c3c; color:white; border:none; border-radius:5px; cursor:pointer;">Tutup & Hapus</button>
                            </div>
                        `).openPopup();
                        ...
                        searchMarker.bindPopup(`
                            <div style="text-align:center;">
                                <strong>Koordinat</strong><br>Lat: ${res.lat}<br>Lng: ${res.lng}
                                <br><button onclick="clearSearchMarker()" style="margin-top:8px; padding:4px 12px; background:#e74c3c; color:white; border:none; border-radius:5px; cursor:pointer;">Tutup & Hapus</button>
                            </div>
                        `).openPopup();
